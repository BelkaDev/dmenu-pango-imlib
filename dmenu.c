...
#endif
#define DEFFONT "TamzenForPowerline"
...
static void savehistory(char *str);
/* HISTORY PATCH */
...
main(int argc, char *argv[]) {
	...
	for(i = 1; i < argc; i++)
		...
		if(!strcmp(argv[i], "-v")) {      /* prints version information */
			...
			use_text_input = 1;
		else if (!strcmp(argv[i], "-H")) /* nav history */
			histfile = argv[++i];
		...
		else if(!strcmp(argv[i], "-ws")) /* wrap selection */
			wrapselection = 1;
...
drawmenu(void) {
	...
	if(prompt && *prompt) {
		dc->w = promptw;
		drawtext(dc, prompt, selcol,True);
		dc->x = dc->w;
	}
	/* draw input field */
	dc->w = (lines > 0 || !matches) ? mw - dc->x : inputw;
	drawtext(dc, text, normcol,True);
...
keypress(XKeyEvent *ev) {
	...
	if(ev->state & ControlMask)
		...
		/*NAVHISTORY_PATCH */
		case XK_Up: navhistory(-1); buf[0]=0; break;
		case XK_Down: navhistory(1); buf[0]=0; break;
		/*NAVHISTORY_PATCH */
		...
		case XK_h: ksym = XK_Up;    break; default:
			return;
		}
	switch(ksym) {
	default:
...
setup(void) {
	...
	if((info = XineramaQueryScreens(dc->dpy, &n))) {
		...
		if(!i && w != root && w != PointerRoot && w != None) {
			/* find top-le	XWindowAttributes wa;vel window containing current input focus */
			do {
...
void
usage(void) {
	fputs("usage: dmenu [-x] [-y] [-h] [-w] [-r] [-b] [-f] [-i] [-g] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
