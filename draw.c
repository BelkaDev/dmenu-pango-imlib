...
void
drawtext(DC *dc, const char *text, ColorSet *col,Bool isPrompt) {
	char buf[BUFSIZ];
	...
	drawtextn(dc, buf, mn, col,isPrompt);
}

void
drawtextn(DC *dc, const char *text, size_t n, ColorSet *col,Bool isPrompt) {
	int x = dc->x + dc->font.height/2;
	int y = dc->y + dc->h/2 - 10;	
	if (isPrompt)
